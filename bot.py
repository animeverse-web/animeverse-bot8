...
def save_to_firebase(anime_id, season, ep_num, quality_dict):
    ...
    if not quality_dict:
        print(f"  ⚠️ Empty dict — skip {ep_key}")
        return ep_key
...
def process_ep(chat_id, ep_num, files):
    ...
    for f in sorted_files:
        ...
        caption = (
            f"🎌 {anime_id}\n"
            f"📺 {season} | {ep_key} | {f['quality']} | {size_mb}MB\n"
            f"━━━━━━━━━━━━━━━━━━━━"
        )
...
📺 `{session.get('anime_id','—')}` | `{session.get('season','—')}`

Naya season: `/setup anime-id S2`
""", parse_mode="Markdown")
...
def cmd_reset(msg):
    if msg.from_user.id != ALLOWED_USER:
        return
    reset_all()
    bot.reply_to(msg, "🔄 *Reset done!*", parse_mode="Markdown")
...
Caption mein `Episode - 04` ya `E04` hona chahiye.
""", parse_mode="Markdown")
        return
    ...
    if file_size in existing_sizes:
        bot.reply_to(msg, f"⚠️ *{ep_key} — Same file dobara aai! Skip.*",
                     parse_mode="Markdown")
        return
